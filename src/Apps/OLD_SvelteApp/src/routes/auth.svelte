<script lang="ts">
    ...
    async function signUpOrSignIn() {
        ...
        if (isSignIn) {
            ...
            const { error, session } = await fetch('https://localhost:5021/api/Login', {
                method: 'POST',
            ...
            const { error, session } = await fetch('https://localhost:5021/api/Login/register', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                credentials: 'include',
                body: JSON.stringify({
...
<div class="flex flex-col justify-center items-center relative">
    ...
    <form class="w-full sm:w-1/2 xl:w-5/12" on:submit|preventDefault={signUpOrSignIn} >
        <div class="border-teal p-8 border-t-12 bg-white mb-6 rounded-lg shadow-lg" style="background: rgba(76, 175, 80, 0.1)">
            ...
            {#if !isSignIn}
                <div class="mb-4">
                    <label for="name" class="block font-semibold text-gray-800 mb-2 text-left">Your name</label>
